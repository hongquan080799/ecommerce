<script lang="ts">
	...
	let filterData = loadedData as Banner[];
	let search = '';
	const onSearch = () => {
		if (search.length > 0) {
			...
			filterData = loadedData as Banner[];
		}
	};
	const onDelete = async (id: number) => {
		const confirmed = confirm('bạn có muốn xóa banner này không ?');
		if (!confirmed) return;
		try {
			await api.bannerApi.deleteBanner(id);
			alert('Xóa banner thành công');
			loadedData = (await api.bannerApi.loadBanners()) as unknown as Banner[];
			onSearch();
		} catch (error) {
			console.log(error);
...
<p class="text-lg font-bold">Banner</p>
...
<div class="relative mt-5 overflow-x-auto shadow-md sm:rounded-lg">
	<div class="flex justify-between bg-white pb-4 dark:bg-gray-900">
		...
		<button
			on:click={() => goto('/admin/banner/create')}
			type="button"
	...
	<table class="w-full text-left text-sm text-gray-500 dark:text-gray-400 rtl:text-right">
		<thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
			<tr>
				...
				<th scope="col" class="px-6 py-3">Mã banner</th>
		...
		<tbody>
			{#each filterData as data}
				...
				>
					...
					<td class="px-6 py-4">
						<button
							on:click={() => goto('/admin/banner/update/' + data.id)}
							type="button"
							class="mb-2 me-2 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
							>Sửa</button
						>
