<script lang="ts">
	...
	let banner: Partial<Banner> = data;
	let imagesPath = banner.imageUrl;
	let isClear = false;
	const clearImage = () => {
		imagesPath = '';
		isClear = true;
	...
	const updateBanner = async () => {
		if (imagesPath !== '') {
			banner.imageUrl = imagesPath;
		}
		try {
			await api.bannerApi.updateBanner(banner);
...
<p class="mb-10 text-lg font-bold">Thêm banner</p>

<form class="form-container" on:submit={updateBanner}>
	<div class="form-group">
		<label for="email" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
			>Tên</label
		>
		<input
			bind:value={banner.name}
			type="text"
			id="firstName"
