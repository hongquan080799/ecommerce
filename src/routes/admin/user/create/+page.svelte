<script lang="ts">
	...
	import { RoleType, type Role, type User } from '../../../../types/User';
	...
	const insertUser = async () => {
		...
		try {
			await api.userApi.insertUser(String(jwt), user);
...
<p class="mb-10 text-lg font-bold">Thêm người dùng</p>

<form class="form-container" on:submit={insertUser}>
	...
	<div class="form-group">
		...
		<Select
			class="mt-2"
			items={roles.map((item) => ({ name: item.name, value: item }))}
